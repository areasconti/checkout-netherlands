...
| --- | --- | --- | --- | --- |
| `olympus` | Partial frontmatter API exists | Tiered bundle selector | `cart-summary01-04` | Reference model from Meridian `theduo-v2`; `bundles:` frontmatter feeds `bundle-selector.html`, and `shipping_methods` keeps Campaigns App shipping refs explicit. Bundle upsell pages now declare `upsell_offer` / `upsell_bundle_tiers` as mapping hints, but still render inline SDK refs until include promotion. |
...
| `olympus` | Bundle upsell pages | Pages declare `upsell_offer` / `upsell_bundle_tiers` frontmatter but still render inline SDK refs. | Partial mapping hint only: replace frontmatter and matching inline package/voucher refs from CampaignSpec/API. | Outside promoted all-page lint today. | Promote to include-owned upsell partials. | P1 |
...
| `shop-three-step` | Upsell and receipt surfaces | Upsell pages declare `upsell_offer` / `upsell_bundle_tiers` frontmatter but still render inline SDK refs; receipt remains inline-heavy. | Partial mapping hint only: replace frontmatter and matching inline package/voucher refs from CampaignSpec/API. | Outside promoted all-page lint today. | Promote to shop-single-step parity includes in a focused follow-up. | P1 |
...
| `olympus-mv-*` | MV and bundle upsell pages | Includes: `upsell-mv-offer.html`, `upsell-bundle-stepper-offer.html`, `upsell-bundle-tier-pills-offer.html`, and `upsell-bundle-tier-cards-offer.html`; used from the matching MV upsell pages. | Yes: `upsell_offer` controls package/selector/button copy, `upsell_mv_tiers` controls MV quantity/voucher tiers, and `upsell_bundle_tiers` controls standard bundle upsell item/voucher tiers. | Outside promoted checkout/select CI today. | Add targeted all-page MV lint once the pages are promotion-gated. | P1 |
...
| `olympus-mv-*` | Receipt/order confirmation | Receipt pages still inline `data-next-skeleton` and `data-next-order-items`. | Partial: fixtures document `receipt_summary`, but the pages have not been promoted to receipt includes. | Outside promoted checkout/select CI; targeted `--pages=all` still fails on receipt only. | Promote MV receipt skeleton and order summary includes in a focused follow-up. | P2 |
