...
| `limos` | Partial frontmatter API exists | Single offer card with native quantity stepper | `cart-summary02` accordion | Roadflare surface promoted to `single-offer-quantity-selector.html`; checkout exposes `single_offer`, `price_display_variant`, and `order_bump_variant`. |
| `demeter` | Partial frontmatter API exists | Editorial tier cards | `cart-summary03` side cart | Veyra surface promoted to `editorial-tier-selector.html`; checkout exposes `price_display_variant`, `order_bump_variant`, and summary fields. |
...
| `olympus-mv-single-step` | MV selector and variant slots | Include: `src/olympus-mv-single-step/_includes/mv-configurable-selector.html`; used from checkout. | Yes: `checkout_step` controls selector/template/stage ids and headings; `variant_slots` controls bundle ids, item JSON, shipping, labels, and selected state. | Yes under `npm run lint:sdk:promoted`. | Add campaign-specific examples after the next MV build. | P1 |
| `olympus-mv-two-step` | Select step and variant slots | Includes: `mv-selection-step.html` and `mv-slot-stage.html`; used from `src/olympus-mv-two-step/select.html`. | Yes: `select_step` and `checkout_step` keep selector/template/stage ids aligned; `variant_slots` controls bundle ids, item JSON, shipping, labels, and selected state. | Yes under `npm run lint:sdk:promoted`. | Add campaign-specific examples after the next MV two-step build. | P1 |
...
3. Mirror Shop single-step upsell/receipt contracts into matching shop-three-step surfaces if parity holds.
4. Add durable headers to remaining non-Shop receipt/upsell surfaces.
...
Do not turn promoted all-page lint into CI yet. Limos, Demeter, Olympus, and Olympus-MV still have inline upsell/receipt SDK roots that should be promoted in smaller family-by-family PRs.
