...
| `shop-three-step` | Upsell selectors/actions | Includes: `upsell-bundle-stepper-offer.html`, `upsell-bundle-tier-pills-offer.html`, and `upsell-bundle-tier-cards-offer.html`; used from the matching upsell pages. | Yes: `upsell_offer` controls package/selector/voucher ids and button copy; `upsell_bundle_tiers` controls tier ids, item JSON, vouchers, labels, and selected state. | Yes under `npm run lint:sdk:promoted`. | Promote receipt summary surfaces separately. | P1 |
...
| `olympus-mv-*` | MV and bundle upsell pages | Includes: `upsell-mv-offer.html`, `upsell-bundle-stepper-offer.html`, `upsell-bundle-tier-pills-offer.html`, and `upsell-bundle-tier-cards-offer.html`; used from the matching MV upsell pages. | Yes: `upsell_offer` controls package/selector/button copy, `upsell_mv_tiers` controls MV quantity/voucher tiers, and `upsell_bundle_tiers` controls standard bundle upsell item/voucher tiers. | Yes under `npm run lint:sdk:promoted`. | Add targeted all-page MV lint once receipt pages are promotion-gated. | P1 |
| `olympus-mv-*` | Checkout payment and summary | Payment, express checkout, and summary includes have catalog wrappers in both MV families. | Partial/no MV-specific summary contract. | Yes under promoted checkout/select rendered scope. | Add summary variant contract and receipt scope. | P2 |
| `olympus-mv-*` | Receipt/order confirmation | Receipt pages still inline `data-next-skeleton` and `data-next-order-items`. | Partial: fixtures document `receipt_summary`, but the pages have not been promoted to receipt includes. | Outside promoted all-page lint; targeted `--pages=all` still fails on receipt only. | Promote MV receipt skeleton and order summary includes in a focused follow-up. | P2 |
...
4. Promote shop-three-step receipt surfaces if parity holds.
5. Add durable headers to remaining Olympus and MV receipt surfaces.
...
Do not turn promoted all-page lint into CI yet. Olympus-MV and shop-three-step still have receipt SDK roots that should be promoted in smaller family-by-family PRs.
